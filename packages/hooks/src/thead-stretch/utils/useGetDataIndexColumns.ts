...
import type { ColumnOriginType } from '../stretch';
...
export function useGetDataIndexColumns<T extends ColumnOriginType<T>>(
  columns: T[] | undefined,
) {
  const dataIndexColumns = useMemo(() => getColumns(columns), [columns]) as
    | T[]
    | undefined;
