...
import type { ColumnOriginType, ColumnsStateType } from '../stretch';
  ...
  if (Array.isArray(res) && Array.isArray(target)) {
    res.forEach((t?, i?) => {
      if (t?.children) {
        ...
        res[i][mergeKey] =
          target.find((x) => x.dataIndex === res[i].dataIndex)?.[mergeKey] ||
          res[i][mergeKey];
      }
    });
  }
...
function useLocalColumns<T extends ColumnOriginType<T>>({
  ...
  const initLocalColumns = useMemoizedFn(() => {
    ...
    try {
      ...
      return mergeColumns<T[]>(
        columnsProp || [],
        localResizableColumns,
        'width',
      );
    } catch (error) {
      console.error(error);
  ...
  const [localColumns, setLocalColumns] = useState<T[] | undefined>(
    initLocalColumns,
  );
